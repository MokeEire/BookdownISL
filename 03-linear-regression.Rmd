...
```{r lab-libraries, message=F, cache=T}
library(MASS)
library(ISLR)
```
...
```{r lab-mlr6, cache=T}
summary(lm(medv~lstat*age, data =))
```
...
We now perform a regression of `medv` onto `lstat` and `lstat`$^\color{#B44C1C}{2}$.
...
```
Here Model 1 represents the linear submodel containing only one predictor, `lstat`, while Model 2 corresponds to the larger quadratic model that has two predictors, `lstat` and `lstat`$\color{#B44C1C}^2$.
...
This provides very clear evidence that the model containing the predictors `lstat` and `lstat`$\color{#B44C1C}^2$ is far superior to the model that only contains the predictor `lstat`. 
...
then we see that when the `lstat`$\color{#B44C1C}^2$ term is included in the model, there is little discernible pattern in the residuals.
