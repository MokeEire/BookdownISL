...
```{r table3-4, fig.align = "center", fig.width= 12, fig.asp = 9/16}
advertising_mlin_reg = lm(sales~TV+radio+newspaper, data = advertising)

reg_table(advertising_mlin_reg) %>% 
  tab_source_note(md(str_c("<span style='color:#1188ce'>TABLE 3.4.</span> ", "*For the ", data_colour("Advertising"), " data, least squares coefficient estimates of the multiple linear regression of number of units sold on radio, TV, and newspaper advertising budgets.*")))
```
...
We use a `r keyword_colour("confidence interval")` to quantify the uncertainty surrounding the *average* `r data_colour("sales")` over a large number of cities.
...
On the other hand, a `r keyword_colour("prediction interval")` can be used to quantify the uncertainty surrounding `r data_colour("sales")` for a *particular* city.
...
For example, the `r data_colour("Credit")` data set displayed in Figure 3.6 records `r data_colour("balance")` (average credit card debt for a number of individuals) as well as several quantitative predictors: `r data_colour("age")`, `r data_colour("cards")` (number of credit cards), `r data_colour("education")` (years of education), `r data_colour("income")` (in thousands of dollars), `r data_colour("limit")` (credit limit), and `r data_colour("rating")` (credit rating).
...
For example, the scatterplot directly to the right of the word "Balance" depicts `r data_colour("balance")` versus `r data_colour("age")`, while the plot directly to the right of "Age" corresponds to `r data_colour("age")` versus `r data_colour("cards")`.
In addition to these quantitative variables, we also have four qualitative variables: `r data_colour("gender")`, `r data_colour("student")` (student status), `r data_colour("status")` (marital status), and `r data_colour("ethnicity")` (Caucasian, African American or Asian).
...
For example, based on the `r data_colour("gender")` variable, we can create a new variable that takes the form
...
For example, for the `r data_colour("ethnicity")` variable we create two dummy variables. The first could be
...
From Table 3.8, we see that the estimated `r data_colour("balance")` for the baseline, African American is $\$531.00$.
...
This F-test has a p-value of $0.96$, indicating that we cannot reject the null hypothesis that there is no relationship between `r data_colour("balance")` and `r data_colour("ethnicity")`.

Using this dummy variable approach presents no difficulties when using both quantitative and qualitative predictors.
For example, to regress `r data_colour("balance")` on both a quantitative variable such as `r data_colour("income")` and a qualitative variable such as `r data_colour("student")`, we must simply create a dummy variable for `r data_colour("student")` and then fit a multiple regression model using `r data_colour("income")` and the dummy variable as predictors for credit card balance.
...
```{r interaction-term-table, fig.align = "center", fig.cap=str_c("*For the ", data_colour("Advertising"), " data, least squares coefficient estimates associated with the regression of ", data_colour("sales"), " onto ", data_colour("TV"), " and ", data_colour("radio"), ", with an interaction term, as in (3.33)*"), fig.width= 12, fig.asp = 9/16}

interaction_model = lm(sales ~ TV+radio+TV*radio, 
                       data = advertising)

reg_table(interaction_model) %>% 
  tab_source_note(md(str_c("<span style='color:#1188ce'>TABLE 3.9.</span>", "*For the ", data_colour("Advertising"), " data, least squares coefficient estimates associated with the regression of ", data_colour("sales"), " onto ", data_colour("TV"), " and ", data_colour("radio"), ", with an interaction term, as in (3.33)*")))
```
...
The p-value for the interaction term `r data_colour("TV")`$\times$`r data_colour("radio")`, is extremely low, indicating there is strong evidence for $H_a : \beta_3 \neq 0$.
In other words, it is clear that the true relationship is not additive.
The $R^2$ for the model \@ref(eq:adv-interaction) is $96.8\%$, compared to only $89.7\%$ for the model that predicts `r data_colour("sales")` using `r data_colour("TV")` and `r data_colour("radio")` without an interaction term.
This means that $(96.8 - 89.7)/(100 - 89.7) = 69\%$ of the variability in `r data_colour("sales")` that remains after fitting the additive model has been explained by the interaction term.
...
In this example, the p-values associated with `r data_colour("TV")`, `r data_colour("radio")`, and the interaction term all are statistically significant (Table 3.9), and so it is obvious that all three variables should be included in the model.
However, it is sometimes the case that an interaction term has a very small p-value, but the associated main effects (in this case, `r data_colour("TV")` and `r data_colour("radio")`) do not.
...
In the previous example, we considered an interaction between `r data_colour("TV")` and `r data_colour("radio")`, both of which are quantitative variables.
...
Consider the `r data_colour("Credit")` data set from Section 3.3.1, and suppose that we wish to predict `r data_colour("balance")` using the `r data_colour("income")` (quantitative) and `r data_colour("student")` (qualitative) variables.
...
The fact that the lines are parallel means that the average effect on `r data_colour("balance")` of a one-unit increase in `r data_colour("income")` does not depend on whether or not the individual is a student.
This represents a potentially serious limitation of the model, since in fact a change in `r data_colour("income")` may have a very different effect on the credit card balance of a student versus a non-student.

This limitation can be addressed by adding an interaction variable, created by multiplying `r data_colour("income")` with the dummy variable for `r data_colour("student")`.
...
The right-hand panel of Figure 3.7 shows the estimated relationships between `r data_colour("income")` and `r data_colour("balance")` for students and non-students in \@ref(eq:qual-interaction).
...
Consider Figure 3.8, in which the `r data_colour("mpg")` (gas mileage in miles per gallon) versus `r data_colour("horsepower")` is shown for a number of cars in the `r data_colour("Auto")` data set.

FIGURE 3.8

The orange line represents the linear regression fit.
There is a pronounced relationship between `r data_colour("mpg")` and `r data_colour("horsepower")`, but it seems clear that this relationship is in fact non-linear: the data suggest a curved relationship.
...
Equation \@ref(eq:mpg-quad) involves predicting `r data_colour("mpg")` using a non-linear function of `r data_colour("horsepower")`.
...
reg_table(mpg_lm) %>% 
  tab_source_note(md(str_c("<span style='color:#1188ce'>TABLE 3.10.</span>", " *For the ", data_colour("Auto"), " data set, least squares coefficient estimates associated with the regression of ", data_colour("mpg"), " onto ", data_colour("horsepower"), " and ", data_colour("horsepower^2"), ".*")))
```
...
The left panel of Figure 3.9 displays a residual plot from the linear regression of `r data_colour("mpg")` onto `r data_colour("horsepower")` on the `r data_colour("Auto")` data set that was illustrated in Figure 3.8.
...
The concept of collinearity is illustrated in Figure 3.14 using the `r data_colour("Credit")` data set.
In the left-hand panel of Figure 3.14, the two predictors `r data_colour("limit")` and `r data_colour("age")` appear to have no obvious relationship.
In contrast, in the right-hand panel of Figure 3.14, the predictors `r data_colour("limit")` and `r data_colour("rating")` are very highly correlated with each other, and we say that they are *collinear*.
The presence of collinearity can pose problems in the regression context, since it can be difficult to separate out the individual effects of collinear variables on the response.
In other words, since `r data_colour("limit")` and `r data_colour("rating")` tend to increase or decrease together, it can be difficult to determine how each one separately is associated with the response, `r data_colour("balance")`.
Figure 3.15 illustrates some of the difficulties that can result from the collinearity.
The left-hand panel of Figure 3.15 is a contour plot of the RSS \@ref(eq:rss) associated with different possible coefficient estimates for the regression of `r data_colour("balance")` on `r data_colour("limit")` and `r data_colour("age")`.
...
The axes for `r data_colour("limit")` and `r data_colour("age")` have been scaled so that the plot includes possible coefficient estimates that are up to four standard errors on either side of the least squares estimates.
Thus the plot includes all plausible values for the coefficients.
For example, we see that the true `r data_colour("limit")` coefficient is almost certainly somewhere between $0.15$ and $0.20$.

FIGURE 3.15

In contrast, the right-hand panel of Figure 3.15 displays contour plots of the RSS associated with possible coefficient estimates for the regression of `r data_colour("balance")` onto `r data_colour("limit")` and `r data_colour("rating")`, which we know to highly collinear.
...
Notice that the scale for the `r data_colour("limit")` coefficient now runs from roughly $-0.2$ to $0.2$; this is an eight-fold increase over the plausible range of the `r data_colour("limit")` coefficient in the regression with `r data_colour("age")`.
Interestingly, even though the `r data_colour("limit")` and `r data_colour("rating")` coefficients now have much more individual uncertainty, they will almost certainly lie somewhere in this contour valley.
For example, we would not expect the true value of the `r data_colour("limit")` and `r data_colour("rating")` coefficients to be $-0.1$ and $1$ respectively, even though such a value is plausible for each coefficient individually.
...
The first is a regression of `r data_colour("balance")` on `r data_colour("age")` and `r data_colour("limit")` and the second is a regression of `r data_colour("balance")` on `r data_colour("rating")` and `r data_colour("limit")`. 
In the first regression both `r data_colour("age")` and `r data_colour("limit")` are highly significant with very small p-values.
In the second, the collinearity between `r data_colour("limit")` and `r data_colour("rating")` has caused the standard error for the `r data_colour("limit")` coefficient estimate to increase by a factor of 12 and the p-value to increase to $0.701$.
In other words, the importance of the `r data_colour("limit")` variable has been masked due to the presence of collinearity.
...
In the `r data_colour("Credit")` data, a regression of `r data_colour("balance")` on `r data_colour("age")`, `r data_colour("rating")`, and `r data_colour("limit")` indicates that the predictors have VIF values of $1.01$, $160.67$, and $160.59$.
...
For instance, if we regress `r data_colour("balance")` onto `r data_colour("age")` and `r data_colour("limit")`, without the `r data_colour("rating")` predictor, then the resulting VIF values are close to the minimum possible value of 1, and the $R^2$ drops from $0.754$ to $0.75$.
So dropping `r data_colour("rating")` from the set of predictors has effectively solved the collinearity problem without compromising the fit.
The second solution is to combine the collinear variables together into a single predictor.
For instance, we might take the average of the standardized versions of `r data_colour("limit")` and `r data_colour("rating")` in order to create a new variable that measures *credit worthiness*.


## The Marketing Plan

We now briefly return to the seven questions about the `r data_colour("Advertising")` data that we set out to answer at the beginning of this chapter.
...
This question can be answered by fitting a multiple regression model of `r data_colour("sales")` onto `r data_colour("TV")`, `r data_colour("radio")`, and `r data_colour("newspaper")`, as in \@ref(eq:mlin-reg-adv), and testing the hypothesis $H_0 : \beta_{\color{#B44C1C}{TV}} = \beta_{\color{#B44C1C}{radio}} = \beta_{\color{#B44C1C}{newspaper}} = 0$. 
...
For the `r data_colour("Advertising")` data, the RSE is $1,681$ units while the mean value for the response is $14,022$, indicating a percentage error of roughly $12\%$.
Second, the $R^2$ statistic records the percentage of variability in the response that is explained by the predictors.
The predictors explain almost $90\%$ of the variance in `r data_colour("sales")`.
...
In the multiple linear regression displayed in Table 3.4, the p-values for `r data_colour("TV")` and `r data_colour("radio")` are low, but the p-value for `r data_colour("newspaper")` is not.
This suggests that only `r data_colour("TV")` and `r data_colour("radio")` are related to `r data_colour("sales")`. 
But the interval for `r data_colour("newspaper")` includes zero, indicating that the variable is not statistically significant given the values of `r data_colour("TV")` and `r data_colour("radio")`.
</details>
...
For the `r data_colour("Advertising")` data, the $95\%$ confidence intervals are as follows: $(0.043, 0.049)$ for `r data_colour("TV")`, $(0.172, 0.206)$ for `r data_colour("radio")`, and $(-0.013, 0.011)$ for `r data_colour("newspaper")`.
The confidence intervals for `r data_colour("TV")` and `r data_colour("radio")` are narrow and far from zero, providing evidnece that these media are related to `r data_colour("sales")`.
But the interval for `r data_colour("newspaper")` includes zero, indicating that the variable is not statistically significant given the values of `r data_colour("TV")` and `r data_colour("radio")`.  
We saw in Section 3.3.3 that collinearity can result in very wide standard errors.
Could collinearity be the reason that the confidence intervals associated with `r data_colour("newspaper")` is so wide?
The VIF scores are $1.005$, $1.145$, and $1.145$ for `r data_colour("TV")`, `r data_colour("radio")`, and `r data_colour("newspaper")`, suggesting no evidence of collinearity.  
...
There is evidence of an extremely strong association between  `r data_colour("TV")` and `r data_colour("sales")` and between `r data_colour("radio")` and `r data_colour("sales")`.
There is evidence of a mild association between `r data_colour("newspaper")` and `r data_colour("sales")`, when the values of `r data_colour("TV")` and `r data_colour("radio")` are ignored.
</details>
...
In the case of the `r data_colour("Advertising")` data, we observe a non-linear effect in Figure 3.5, though this effect could also be observed in a residual plot.
...
Figure 3.5 suggested that the `r data_colour("Advertising")` data may not be additive.
