...
Recall the `r data_color("Advertising")` data from Chapter 2. 
Figure 2.1 displays `r data_color("sales")` (in thousands of units) for a particular product as a function of advertising budgets (in thousands of dollars) for `r data_color("TV")`, `r data_color("radio")`, and `r data_color("newspaper")` media. 
...
For example, *X* may represent `r data_color("TV")` advertising and *Y* may represent `r data_color("sales")`. 
Then we can regress `r data_color("sales")` onto `r data_color("TV")` by fitting the model:
...
In the `r data_color("Advertising")` example, this data set consists of the TV advertising budget and product sales in $n = 200$ different markets.
...
```{r advertising-lm, fig.align = "center", fig.cap=paste("*For the", data_color("Advertising"), "data, the least squares fit for the regression of", data_color("sales"), "onto", data_color("TV"), "is shown.  The fit is found by minimizing the sum of squared errors.  Each grey line segment represents an error, and the fit makes a compromise by averaging their squares.  In this case a linear fit captures the essence of the relationship, although it is somewhat deficient in the left of the plot.*"), fig.width= 12, fig.asp = 9/16}
...
\ref(fig:advertising-lm) displays the simple linear regression fit to the `r data_color("Advertising")` data, where $\hat{\beta_0} = 7.03$ and $\hat{\beta_1} = 0.0475$.
...
In **Figure 3.2**, we have computed RSS for a number of values of $\beta_0$ and $\beta_1$, using the advertising data with `r data_color("sales")` as the response and `r data_color("TV")` as the predictor.
